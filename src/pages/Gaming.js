...
function Gaming() {
  ...
  const handleAnswerOptionClick = (isCorrect) => {
    ...
    if (nextQuestion < data.Gaming.length) {
        ...
        <>
          <div className="question-section">
            <div className="question-count">
              <span>Question {currentQuiestion + 1}</span>/{data.Gaming.length}
            </div>
            <div className="question-text">
              {data.Gaming[currentQuiestion].questionText}
            </div>
            <div className="answer-section">
              {data.Gaming[currentQuiestion].answerOptions.map(
                (answerOption, idx) => (
                  <button
                    key={idx}
